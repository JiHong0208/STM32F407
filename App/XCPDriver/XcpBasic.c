...
V_MEMROM0 const vuint8 kXcpMainVersion    = (vuint8)(CP_XCP_VERSION >> 8u);
V_MEMROM0 const vuint8 kXcpSubVersion     = (vuint8)(CP_XCP_VERSION & 0x00ffu);
...
#else
void XcpSendDto(/*const*/ tXcpDto* Xcpdto)
{
    ApplXcpSend(Xcpdto->l, &Xcpdto->b[0]);
}
#endif
...
static void XcpFreeDaq(void)
{
    ...
    xcp.pOdt = (tXcpOdt*)0;
    xcp.pOdtEntryAddr = (DAQBYTEPTR*)0;
    xcp.pOdtEntrySize = (vuint8*)0;
    ...
    XcpMemSet((BYTEPTR)&xcp.Daq.EventDaq[0], (vuint16)sizeof(xcp.Daq.EventDaq),
              (vuint8)0xFFu);   /* Deviation of MISRA rule 44. */
#endif
#if defined ( XCP_ENABLE_SEND_QUEUE )
    XcpQueueInit();
#endif
...
#if defined ( C_CPUTYPE_8BIT ) || defined ( C_CPUTYPE_16BIT )
    /* Align to words */
    xcp.pOdt = (tXcpOdt*)((((vuint32)(&xcp.Daq.u.DaqList[xcp.Daq.DaqCount])) + (vuint32)1u) &
                          (vuint32)0xFFFFFFFEu); /* PRQA S 0306 */ /* MD_Xcp_0306 */
#else
    /* Align to dwords */
    xcp.pOdt = (tXcpOdt*)((((vuint32)(&xcp.Daq.u.DaqList[xcp.Daq.DaqCount])) + (vuint32)3u) &
                          (vuint32)0xFFFFFFFCu); /* PRQA S 0306 */ /* MD_Xcp_0306 */
#endif
#endif
#else
...
#else
    xcp.pOdtEntryAddr = (DAQBYTEPTR*)((((vuint32)&xcp.pOdt[xcp.Daq.OdtCount]) + (vuint32)3u) &
                                      (vuint32)0xFFFFFFFCu); /* PRQA S 0306 */ /* MD_Xcp_0306 */
#endif
#else
...
#if defined ( C_CPUTYPE_8BIT ) || defined ( C_CPUTYPE_16BIT )
    /* Allign to words */
    xcp.pQueue = (tXcpDto*)((((vuint32)(&xcp.pOdtEntrySize[xcp.Daq.OdtEntryCount])) + (vuint32)1u) &
                            (vuint32)0xFFFFFFFEu); /* PRQA S 0306 */ /* MD_Xcp_0306 */
#else
    /* Allign to dwords */
    xcp.pQueue = (tXcpDto*)((((vuint32)(&xcp.pOdtEntrySize[xcp.Daq.OdtEntryCount])) + (vuint32)3u) &
                            (vuint32)0xFFFFFFFCu); /* PRQA S 0306 */ /* MD_Xcp_0306 */
#endif
#endif
#else
            ...
            switch (CRO_CMD)
            {
                ...
                case CC_GET_SEED:
                {
                    ...
                    if (gDebugLevel != 0)
                    {
                        ...
                        ApplXcpPrint("<- 0xFF length=%02Xh, seed=%02X%02X%02X%02X%02X%02X\n", CRM_GET_SEED_LENGTH, CRM_GET_SEED_DATA[0],
                                     CRM_GET_SEED_DATA[1], CRM_GET_SEED_DATA[2], CRM_GET_SEED_DATA[3], CRM_GET_SEED_DATA[4], CRM_GET_SEED_DATA[5]);
                    }
                ...
                case CC_UNLOCK:
                {
                    ...
                    if (gDebugLevel != 0)
                    {
                        ApplXcpPrint("-> UNLOCK key=%02X%02X%02X%02X%02X%02X\n", CRO_UNLOCK_KEY[0], CRO_UNLOCK_KEY[1], CRO_UNLOCK_KEY[2],
                                     CRO_UNLOCK_KEY[3], CRO_UNLOCK_KEY[4], CRO_UNLOCK_KEY[5]);
                    }
                ...
                case CC_SET_CAL_PAGE:
                {
                    ...
                    if (gDebugLevel != 0)
                    {
                        ApplXcpPrint("-> SET_CAL_PAGE segment=%u,page =%u,mode=%02Xh\n", CRO_SET_CAL_PAGE_SEGMENT, CRO_SET_CAL_PAGE_PAGE,
                                     CRO_SET_CAL_PAGE_MODE);
                    }

#endif
                    check_error(ApplXcpSetCalPage(CRO_SET_CAL_PAGE_SEGMENT, CRO_SET_CAL_PAGE_PAGE,
                                                  CRO_SET_CAL_PAGE_MODE)) /* PRQA S 2001 */ /* MD_Xcp_2001 */
#if defined ( XCP_ENABLE_TESTMODE )
                    ...
                    */
                    ...
                    if (gDebugLevel && CRO_GET_SEGMENT_INFO_MODE == 1)
                    {
                        ApplXcpPrint("<- 0xFF Mode=%u, Maxpages=%u, AddrExt=%u, Mappingmax=%u, Compression=%u, Encryption=%u\n",
                                     CRO_GET_SEGMENT_INFO_MODE, CRM_GET_SEGMENT_INFO_MAX_PAGES, CRM_GET_SEGMENT_INFO_ADDRESS_EXTENSION,
                                     CRM_GET_SEGMENT_INFO_MAX_MAPPING, CRM_GET_SEGMENT_INFO_COMPRESSION, CRM_GET_SEGMENT_INFO_ENCRYPTION);
                    }
                ...
                case CC_COPY_CAL_PAGE:
                {
                    ...
                    err = ApplXcpCopyCalPage(CRO_COPY_CAL_PAGE_SRC_SEGMENT, CRO_COPY_CAL_PAGE_SRC_PAGE, CRO_COPY_CAL_PAGE_DEST_SEGMENT,
                ...
                case CC_SHORT_UPLOAD:
                {
                    ...
                    if (gDebugLevel != 0)
                    {
                        ApplXcpPrint("-> SHORT_UPLOAD addr=%08Xh, addrext=%02Xh, size=%u\n", CRO_SHORT_UPLOAD_ADDR, CRO_SHORT_UPLOAD_EXT,
                                     CRO_SHORT_UPLOAD_SIZE);
                    }
...
( kXcpChecksumMethod == XCP_CHECKSUM_TYPE_ADD44 )
...
#endif
                        {
                            ...
                            /*jackyu*/
                            /*This is a bug from Vector code*/
                            xcp.CrmLen = CRM_BUILD_CHECKSUM_LEN;/*xcp.CrmLen default is last command length, we need value is 8*/
                            /*jackyu*/
#else
...
#endif
                            ;
                    CRM_GET_DAQ_RESOLUTION_INFO_TIMESTAMP_TICKS_WRITE(
                                    kXcpDaqTimestampTicksPerUnit);  /* BCD coded */ /* PRQA S 3109 */ /* MD_MSR_14.3 */
#else
                ...
                case CC_GET_DAQ_EVENT_INFO: /* PRQA S 2003 */ /* MD_Xcp_2003 */
                {
                    ...
                    if (gDebugLevel != 0)
                    {
                        ApplXcpPrint("<- 0xFF name=%s, unit=%u, cycle=%u\n", kXcpEventName[event], CRM_GET_DAQ_EVENT_INFO_TIME_UNIT,
                                     CRM_GET_DAQ_EVENT_INFO_TIME_CYCLE);
                    }
                ...
                case CC_GET_DAQ_LIST_MODE:
                {
                    ...
                    if (gDebugLevel != 0)
                    {
                        ApplXcpPrint("<- 0xFF mode=%02X, prescaler=%u, eventChannel=%u, priority=%u, /*maxOdtEntrySize=%u*/  \n",
                                     CRM_GET_DAQ_LIST_MODE_MODE, CRM_GET_DAQ_LIST_MODE_PRESCALER, CRM_GET_DAQ_LIST_MODE_EVENTCHANNEL,
                                     CRM_GET_DAQ_LIST_MODE_PRIORITY);
                    }
                ...
                {
                    CheckResourceProtection(RM_PGM);    /*must in RM_PGM mode*/
                    xcp.CrmLen = CRM_PROGRAM_START_LEN;
                    CRM_PROGRAM_MAX_CTO_PGM = kXcpMaxCTO;
                    CRM_PROGRAM_COMM_MODE_PGM = (vuint8)0;
                    ...
                    CRM_PROGRAM_MAX_BS_PGM = (vuint8)((254u / CRO_PROGRAM_MAX_SIZE) + (vuint8)1u);
                    CRM_PROGRAM_MIN_ST_PGM = (vuint8)kXcpProgramMinStPgm;
#else
                    CRM_PROGRAM_MAX_BS_PGM = 0u;
                    CRM_PROGRAM_MIN_ST_PGM = 0u;
#endif
                    CRM_PROGRAM_QUEUE_SIZE_PGM = (vuint8)0u;/*inter leaved mode is not implement*/
#if defined ( XCP_ENABLE_BOOTLOADER_DOWNLOAD )
#else
                    err = ApplXcpProgramStart();

                    if (XCP_CMD_PENDING == err)
                    {
                        goto no_response;
                    }
...
#endif
#if defined ( XCP_ENABLE_PROGRAMMING_WRITE_PROTECTION )
                    ret = ApplXcpCheckProgramAccess(xcp.Mta, size);

                    if (XCP_CMD_DENIED == ret)
                    {
                        xcp.Mta += size;
                        error(CRC_ACCESS_DENIED);
                    }
...
#endif
                    ret = ApplXcpFlashProgram(CRO_PROGRAM_DATA, xcp.Mta, size);
                    xcp.Mta += size;
                    xcp.CrmLen = CRM_PROGRAM_LEN;

                    if (XCP_CMD_PENDING == ret)
                    {
                        goto no_response;
                    }
